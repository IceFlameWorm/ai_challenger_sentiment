...
from keras.layers import Dropout, BatchNormalization, SpatialDropout1D, Concatenate
...
class LinXiSingleModel(SingleModel):
    def _build(self, *args, **kwargs):
        ...
        if embedding is None:
            out = Embedding(50000, 100)(inp)
        else:
            out = Embedding(50000, 100, weights = [embedding], trainable = False)(inp)
        #out = SpatialDropout1D(0.5)(out)
        out = Conv1D(64, 3, activation='relu')(out)
        out = MaxPooling1D(5)(out)
        #out = BatchNormalization()(out)
...
#        out = Concatenate()([out1, out2, out3])
    ...
    def fit(self, inputs, outputs, *args, **kwargs):
        ...
        train_sample_weights = class_weight.compute_sample_weight('balanced', y_train_onehot)

        model_file = kwargs['model_file']
        
        self._model.compile(optimizer= Adam(lr),loss='categorical_crossentropy',metrics=['acc'])
        cbs = [
#                ModelCheckpoint(model_file, monitor= 'val_acc', save_best_only=True, mode = 'max'),
                CustomModelCheckPoint(model_file,
                                      monitor = self._f1_monitor,
                                      save_best_only = True,
                                      mode = 'max',
                                      verbose = 1,
                                      val_data = (val_x, y_val_onehot)),
                EarlyStopping(patience=PATIENCE),
                ReduceLROnPlateau('val_loss', factor=0.1, verbose=1, patience=int(PATIENCE / 2)),
               #CyclicLR(base_lr = lr, max_lr = 10 * lr, step_size = 1000,
                        #mode = 'exp_range', gamma=0.99994
               #        )
              ]
...
class LinXiCompositeModel(CompositeModel):
    def fit(self, inputs, outputs, *args, **kwargs):
        ...
        train_x = inputs
        SAVED_PATH = os.path.join(SAVED_MODELS_PATH, 'linxi')
        ...
        for i, comp in enumerate(self._comps):
            ...
            y_train_onehot = to_categorical(train_y)
            y_val_onehot = to_categorical(val_y)
            MODEL_PRE = os.path.join(SAVED_PATH, 'cnn_')
...
#        xs = np.array(inputs[seq].values.tolist(), dtype = np.int)
        ...
        outs = super(LinXiCompositeModel, self).predict(inputs)
        return outs
